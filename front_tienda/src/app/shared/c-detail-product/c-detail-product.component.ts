...
export class CDetailProductComponent implements OnInit {
  ...
  ngOnInit(): void {
    setTimeout(() => {
      ...
      //si es ventana no muestra scroll
      if(!this.is_landing){
  ...
  getNewPriceS(price_colones:number,discount_g:any) {
    if(discount_g.type_discount == 1){ //%
      return price_colones - (price_colones*discount_g.discount*0.01);
    }
    if(discount_g.type_discount == 2){ //CRC
      return price_colones - discount_g.discount;
    }
    return 0;
  }
  addCart(product_selected_modal:any){
    ...
    //inventario multiple
    if(this.product_selected_modal.checked_inventario == 2){
    ...
    //inventario unitario
    if(this.product_selected_modal.checked_inventario == 1){
      if(this.quantity <= 0){
        alertDanger("NECESITAS INGRESAR UNA CANTIDAD");
        return;
      }
    ...
    if(this.product_selected_modal.discount_g){
      ...
      precio_uni_total = this.getNewPriceS(this.product_selected_modal.price_colones,this.product_selected_modal.discount_g);
    }else{
      precio_uni_total = this.product_selected_modal.price_colones;
    }
    //declaracion de los datos dispersos requeridos
    let data = {
    ...
    //respuesta del api si todo fue correcto.
