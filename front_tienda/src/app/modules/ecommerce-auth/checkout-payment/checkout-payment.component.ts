...
export class CheckoutPaymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._cartService.ToDolar().subscribe((resp: any) => {
      ...
      console.log(this.ConversationDolar + 'ckeckout dolar');
    })
    ...
    paypal.Buttons({
      ...
      createOrder: (data:any, actions:any) => {
          ...
          const createOrderPayload = {
            purchase_units: [
              {
                  intent: "JANUARY 29 - Compra", // Aquí puedes agregar el nombre de tu empresa y una descripción general de la compra
                  amount: {
  ...
  save() {
    if (!this.full_name || !this.full_surname) {
      alertDanger("NECESITAS INGRESAR EL NOMBRE Y APELLIDO DEL QUE RECIBE EL PAQUETE O ENTREGA");
      return;
    }
  ...
  PROCESS_PAYMENT() {
    ...
    if (!this.date_expiration) {
      alertDanger("NECESITAS COLOCAR LA FECHA DE EXPIRACIÓN DE LA TARJETA");
      return;
    }
    if (!this.cvv) {
