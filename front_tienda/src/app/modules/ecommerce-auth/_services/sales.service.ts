...
export class SalesService {
  ...
  storeSale(data:any){
    ...
    let URL = URL_SERVICES + "/ecommerce/checkout/sale";
    return this.http.post(URL,data,{headers: headers});
  }
  // CHECKOUT - DIRECCION DEL CLIENTE
  listAddressUser(){
    let headers = new HttpHeaders({'Authorization': 'Bearer ' + this._authServices.token});
    let URL = URL_SERVICES + "/ecommerce/checkout/address_user";
  ...
  updateAddressUser(cart_id:any,data:any){
    let headers = new HttpHeaders({'Authorization': 'Bearer ' + this._authServices.token});
    let URL = URL_SERVICES + "/ecommerce/checkout/address_user/"+cart_id;
