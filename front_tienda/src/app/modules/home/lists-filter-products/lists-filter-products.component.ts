...
export class ListsFilterProductsComponent implements OnInit {
  ...
  itemsPerPage: number = 10; // o cualquier otro número que desees
  totalItems: number = 0; // Inicialmente es 0, pero lo actualizaremos más tarde
  totalPages: number = 0; // También lo actualizaremos más tarde
  ...
  listProducts(){
    ...
    this._homeServices.listProducts(data).subscribe((resp:any) => {
      ...
      // Actualizar las variables de paginación
  ...
  getNewPriceS(price_colones:number,discount_g:any) {
    ...
    if(discount_g.type_discount == 2){ //CRC
      return price_colones - price_colones*discount_g.discount;
    }
    return 0;
  }
  ...
  addCart(product_selected_modal:any){
    ...
    if(product_selected_modal.discount_g){
      ...
      precio_uni_total = this.getNewPriceS(product_selected_modal.price_colones,product_selected_modal.discount_g);
    }else{
      precio_uni_total = product_selected_modal.price_colones;
    }
    let data = {
